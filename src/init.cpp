...
void Shutdown()
{
    ...
    RenameThread("kittycoin-shutoff");
...
std::string HelpMessage()
{
    ...
    strUsage += "  -conf=<file>           " + _("Specify configuration file (default: kittycoin.conf)") + "\n";
    strUsage += "  -pid=<file>            " + _("Specify pid file (default: kittycoind.pid)") + "\n";
...
strUsage += "\n" + _("Masternode options:") + "\n";
    ...
    strUsage += "  -anonymizekittycoinamount=<n> " + _("Keep N kittycoin anonymized (default: 0)") + "\n";
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. kittycoin is shutting down."));
    ...
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. kittycoin is probably already running."), strDataDir));

    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("kittycoin version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
    ...
    if (fDaemon)
        fprintf(stdout, "kittycoin server starting\n"); 
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK)
        {
            ...
            else if (nLoadWalletRet == DB_NONCRITICAL_ERROR)
            {
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of kittycoin") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE)
            {
                strErrors << _("Wallet needed to be rewritten: restart kittycoin to complete") << "\n";
    ...
    nAnonymizekittycoinAmount = GetArg("-anonymizekittycoinamount", 0);
    if(nAnonymizekittycoinAmount > 999999) nAnonymizekittycoinAmount = 999999;
    if(nAnonymizekittycoinAmount < 2) nAnonymizekittycoinAmount = 2;
    ...
    LogPrintf("Anonymize kittycoin Amount %d\n", nAnonymizekittycoinAmount);

    /* Denominations
       A note about convertability. Within Darksend pools, each denomination
       is convertable to another.
       For example:
       1KTY+1000 == (.1KTY+100)*10
       10KTY+10000 == (1KTY+1000)*10
    */
