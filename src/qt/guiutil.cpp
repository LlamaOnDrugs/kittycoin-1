...
bool parseBitcoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    ...
    if(uri.scheme() != QString("kittycoin"))
        return false;
...
{
    // Convert kittycoin:// to kittycoin:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("kittycoin://", Qt::CaseInsensitive))
    {
        uri.replace(0, 11, "kittycoin:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "kittycoin.lnk";
}

bool GetStartOnSystemStartup()
{
    // check for kittycoin.lnk
...
{
    return GetAutostartDir() / "kittycoin.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        // Write a kittycoin.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        optionFile << "Name=kittycoin\n";
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the kittycoin app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if(fAutoStart && !foundItem) {
        // add kittycoin app to startup item list
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("kittycoin-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  kittycoin-qt [" + tr("command-line options") + "]                     " + "\n";
    ...
    setWindowTitle(tr("kittycoin-Qt"));
