...
void OverviewPage::updateDarksendProgress()
{
    ...
    QString strAnonymizekittycoinAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizekittycoinAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if(currentBalance == 0)
    {
        ...
        strAnonymizekittycoinAmount = strAnonymizekittycoinAmount.remove(strAnonymizekittycoinAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizekittycoinAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    ...
    if(nMaxToAnonymize > nAnonymizekittycoinAmount*COIN) nMaxToAnonymize = nAnonymizekittycoinAmount*COIN;

    if(nMaxToAnonymize == 0) return;

    if(nMaxToAnonymize >= nAnonymizekittycoinAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                          .arg(strAnonymizekittycoinAmount));
        strAnonymizekittycoinAmount = strAnonymizekittycoinAmount.remove(strAnonymizekittycoinAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizekittycoinAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    } else {
...
void OverviewPage::toggleDarksend(){
    ...
    if(!fEnableDarksend){
        ...
        if(nAnonymizekittycoinAmount == 0){
