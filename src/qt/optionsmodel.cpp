...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nAnonymizekittycoinAmount"))
        settings.setValue("nAnonymizekittycoinAmount", 1000);
    nAnonymizekittycoinAmount = settings.value("nAnonymizekittycoinAmount").toLongLong();
    if (settings.contains("nDarksendRounds"))
        SoftSetArg("-darksendrounds", settings.value("nDarksendRounds").toString().toStdString());
    if (settings.contains("nAnonymizekittycoinAmount"))
        SoftSetArg("-anonymizetraveamount", settings.value("nAnonymizekittycoinAmount").toString().toStdString());
...
QVariant OptionsModel::data(const QModelIndex & index, int role) const
{
    if(role == Qt::EditRole)
    {
        ...
        case AnonymizekittycoinAmount:
            return QVariant(nAnonymizekittycoinAmount);
        case UseBlackTheme:
...
bool OptionsModel::setData(const QModelIndex & index, const QVariant & value, int role)
{
    ...
    if(role == Qt::EditRole)
    {
        ...
        case DarksendRounds:
            ...
            break;
        case AnonymizekittycoinAmount:
            nAnonymizekittycoinAmount = value.toInt();
            settings.setValue("nAnonymizekittycoinAmount", nAnonymizekittycoinAmount);
            emit AnonymizekittycoinAmountChanged(nAnonymizekittycoinAmount);
            break;
        default:
            break;
        }
