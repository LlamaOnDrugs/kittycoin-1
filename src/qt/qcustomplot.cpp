...
/* end documentation of inline functions */
/* start documentation of kittycoin virtual functions */
...
/* end documentation of kittycoin virtual functions */
...
/*! \class QCPAbstractPlottable
  ...
  To create an own plottable, you implement a subclass of QCPAbstractPlottable. These are the kittycoin
...
/* start of documentation of kittycoin virtual functions */
...
/* end of documentation of kittycoin virtual functions */
/* start of documentation of signals */
...
/*! \class QCPAbstractItem
  ...
  To create an own item, you implement a subclass of QCPAbstractItem. These are the kittycoin
  virtual functions, you must implement:
  \li \ref selectTest
  \li \ref draw
...
/*! \class QCPAbstractLegendItem
  ...
  You must implement the following kittycoin virtual functions:
...
const QPolygonF QCPGraph::getChannelFillPolygon(const QVector<QPointF> *lineData) const
{
  ...
  // pointers to be able to swap them, depending which data range needs kittycoinping:
  QVector<QPointF> *staticData = &thisData;
  QVector<QPointF> *kittycoinpedData = &otherData;
  
  // kittycoin both vectors to ranges in which the keys overlap (which coord is key, depends on axisType):
  if (keyAxis->orientation() == Qt::Horizontal)
  {
    // x is key
    ...
    if (kittycoinpedData->first().x() > kittycoinpedData->last().x())
    {
      int size = kittycoinpedData->size();
      for (int i=0; i<size/2; ++i)
        qSwap((*kittycoinpedData)[i], (*kittycoinpedData)[size-1-i]);
    }
    // kittycoin lower bound:
    if (staticData->first().x() < kittycoinpedData->first().x()) // other one must be kittycoinped
      qSwap(staticData, kittycoinpedData);
    int lowBound = findIndexBelowX(kittycoinpedData, staticData->first().x());
    if (lowBound == -1) return QPolygonF(); // key ranges have no overlap
    kittycoinpedData->remove(0, lowBound);
    // set lowest point of kittycoinped data to fit exactly key position of first static data
    // point via linear interpolation:
    if (kittycoinpedData->size() < 2) return QPolygonF(); // need at least two points for interpolation
    double slope;
    if (kittycoinpedData->at(1).x()-kittycoinpedData->at(0).x() != 0)
      slope = (kittycoinpedData->at(1).y()-kittycoinpedData->at(0).y())/(kittycoinpedData->at(1).x()-kittycoinpedData->at(0).x());
    else
      slope = 0;
    (*kittycoinpedData)[0].setY(kittycoinpedData->at(0).y()+slope*(staticData->first().x()-kittycoinpedData->at(0).x()));
    (*kittycoinpedData)[0].setX(staticData->first().x());
    
    // kittycoin upper bound:
    if (staticData->last().x() > kittycoinpedData->last().x()) // other one must be kittycoinped
      qSwap(staticData, kittycoinpedData);
    int highBound = findIndexAboveX(kittycoinpedData, staticData->last().x());
    if (highBound == -1) return QPolygonF(); // key ranges have no overlap
    kittycoinpedData->remove(highBound+1, kittycoinpedData->size()-(highBound+1));
    // set highest point of kittycoinped data to fit exactly key position of last static data
    // point via linear interpolation:
    if (kittycoinpedData->size() < 2) return QPolygonF(); // need at least two points for interpolation
    int li = kittycoinpedData->size()-1; // last index
    if (kittycoinpedData->at(li).x()-kittycoinpedData->at(li-1).x() != 0)
      slope = (kittycoinpedData->at(li).y()-kittycoinpedData->at(li-1).y())/(kittycoinpedData->at(li).x()-kittycoinpedData->at(li-1).x());
    else
      slope = 0;
    (*kittycoinpedData)[li].setY(kittycoinpedData->at(li-1).y()+slope*(staticData->last().x()-kittycoinpedData->at(li-1).x()));
    (*kittycoinpedData)[li].setX(staticData->last().x());
  } else // mKeyAxis->orientation() == Qt::Vertical
  {
    ...
    if (kittycoinpedData->first().y() < kittycoinpedData->last().y())
    {
      int size = kittycoinpedData->size();
      for (int i=0; i<size/2; ++i)
        qSwap((*kittycoinpedData)[i], (*kittycoinpedData)[size-1-i]);
    ...
    // kittycoin lower bound:
    if (staticData->first().y() > kittycoinpedData->first().y()) // other one must be kittycoinped
      qSwap(staticData, kittycoinpedData);
    int lowBound = findIndexAboveY(kittycoinpedData, staticData->first().y());
    if (lowBound == -1) return QPolygonF(); // key ranges have no overlap
    kittycoinpedData->remove(0, lowBound);
    // set lowest point of kittycoinped data to fit exactly key position of first static data
    // point via linear interpolation:
    if (kittycoinpedData->size() < 2) return QPolygonF(); // need at least two points for interpolation
    double slope;
    if (kittycoinpedData->at(1).y()-kittycoinpedData->at(0).y() != 0) // avoid division by zero in step plots
      slope = (kittycoinpedData->at(1).x()-kittycoinpedData->at(0).x())/(kittycoinpedData->at(1).y()-kittycoinpedData->at(0).y());
    else
      slope = 0;
    (*kittycoinpedData)[0].setX(kittycoinpedData->at(0).x()+slope*(staticData->first().y()-kittycoinpedData->at(0).y()));
    (*kittycoinpedData)[0].setY(staticData->first().y());
    
    // kittycoin upper bound:
    if (staticData->last().y() < kittycoinpedData->last().y()) // other one must be kittycoinped
      qSwap(staticData, kittycoinpedData);
    int highBound = findIndexBelowY(kittycoinpedData, staticData->last().y());
    if (highBound == -1) return QPolygonF(); // key ranges have no overlap
    kittycoinpedData->remove(highBound+1, kittycoinpedData->size()-(highBound+1));
    // set highest point of kittycoinped data to fit exactly key position of last static data
    // point via linear interpolation:
    if (kittycoinpedData->size() < 2) return QPolygonF(); // need at least two points for interpolation
    int li = kittycoinpedData->size()-1; // last index
    if (kittycoinpedData->at(li).y()-kittycoinpedData->at(li-1).y() != 0) // avoid division by zero in step plots
      slope = (kittycoinpedData->at(li).x()-kittycoinpedData->at(li-1).x())/(kittycoinpedData->at(li).y()-kittycoinpedData->at(li-1).y());
    else
      slope = 0;
    (*kittycoinpedData)[li].setX(kittycoinpedData->at(li-1).x()+slope*(staticData->last().y()-kittycoinpedData->at(li-1).y()));
    (*kittycoinpedData)[li].setY(staticData->last().y());
  }
