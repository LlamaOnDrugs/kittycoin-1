...
int nAnonymizekittycoinAmount = 1000;
...
#else
    const char* pszModule = "kittycoin";
#endif
    if (pex)
        return strprintf(
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\kittycoin
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\kittycoin
    // Mac: ~/Library/Application Support/kittycoin
    // Unix: ~/.kittycoin
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "kittycoin";
#else
    fs::path pathRet;
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "kittycoin.conf"));
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "kittycoind.pid"));
